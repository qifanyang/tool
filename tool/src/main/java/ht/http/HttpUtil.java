...
public class HttpUtil {
	...
	public static ChannelBuffer encodeResponse(HttpResponseStatus status,
			byte[] content, List<String> cookies) {
		...
		HttpHeaders.setHeader(response, HttpHeaders.Names.SET_COOKIE, cookies);
	...
	public static ChannelBuffer encodeResponse(HttpResponseStatus status,
		...
		HttpHeaders.addHeader(response, HttpHeaders.Names.CONTENT_TYPE,"text/html; charset=utf-8");

		ChannelBuffer buffer = new BigEndianHeapChannelBuffer(content);
		response.setContent(buffer);
		HttpHeaders.setHeader(response, HttpHeaders.Names.CONTENT_LENGTH,content.length);
		return encodeResponse(response);
	}
	...
	public static ChannelBuffer encodeResponse(HttpResponseStatus status,
			String content) {
		return encodeResponse(status, content.getBytes(CharsetUtil.UTF_8));
	}

	public static ChannelBuffer encodeResponse(HttpResponseStatus status,
			String content, List<String> cookies) {
		return encodeResponse(status, content.getBytes(CharsetUtil.UTF_8),
				cookies);
	}

	public static ChannelBuffer codeOnly(HttpResponseStatus status) {
		return encodeResponse(status, new byte[0]);
	}
